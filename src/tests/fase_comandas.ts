...
async function main() {
  ...
  // ─── Mesas ───
  const tableA = await unwrap<{ id: number }>(await api('/api/comandas/tables', { method: 'POST', body: JSON.stringify({ label: 'Mesa 1' }) }, admin!));
  const tableB = await unwrap<{ id: number }>(await api('/api/comandas/tables', { method: 'POST', body: JSON.stringify({ label: 'Mesa 2' }) }, admin!));
  const tableC = await unwrap<{ id: number }>(await api('/api/comandas/tables', { method: 'POST', body: JSON.stringify({ label: 'Mesa 3' }) }, admin!));
  check('3 mesas criadas', !!tableA.id && !!tableB.id && !!tableC.id);
  const statusInit = await unwrap<{ id: number; status: string }[]>(await api('/api/comandas/tables/status', {}, admin!));
  check('mesas comecam livres', statusInit.every((t) => t.status === 'livre'));

  // ─── Produtos: normal, kit (Fase 2c) e produzido (Fase 2d) ───
  const suco = await unwrap<{ id: number }>(await api('/api/commercial/products', { method: 'POST', body: JSON.stringify({ name: 'Suco de Laranja', priceCents: 800 }) }, admin!));
  const hamburguer = await unwrap<{ id: number }>(await api('/api/commercial/products', { method: 'POST', body: JSON.stringify({ name: 'Hamburguer', priceCents: 1200, costCents: 400 }) }, admin!));
  ...
  const kit = await unwrap<{ id: number }>(await api('/api/commercial/products', { method: 'POST', body: JSON.stringify({ name: 'Combo Lanche', priceCents: 2500, costCents: 1000, productType: 'kit' }) }, admin!));
  ...
  const laranja = await unwrap<{ id: number }>(await api('/api/commercial/products', { method: 'POST', body: JSON.stringify({ name: 'Laranja kg', priceCents: 200, costCents: 50 }) }, admin!));
  ...
  const produzido = await unwrap<{ id: number }>(await api('/api/commercial/products', { method: 'POST', body: JSON.stringify({ name: 'Suco Natural 300ml', priceCents: 900, productType: 'produzido' }) }, admin!));
  ...
  const comanda = await unwrap<{ id: number }>(openR);
  ...
  const comandaAfterAdd = await unwrap<{ items: ComandaItem[] }>(await api(`/api/comandas/comandas/${comanda.id}`, {}, admin!));
  ...
  const comandaAfterPriceChange = await unwrap<{ items: ComandaItem[] }>(await api(`/api/comandas/comandas/${comanda.id}`, {}, admin!));
  ...
  const descartavel = await unwrap<{ id: number }>(addDescartavel);
  ...
  const comandaAfterVoid = await unwrap<{ items: ComandaItem[] }>(await api(`/api/comandas/comandas/${comanda.id}`, {}, admin!));
  ...
  const statusAfterTransfer = await unwrap<{ id: number; status: string }[]>(await api('/api/comandas/tables/status', {}, admin!));
  ...
  const split = await unwrap<{ newComandaId: number }>(splitR);
  check('split OK', splitR.status === 200 && !!split.newComandaId);
  const comandaAfterSplit = await unwrap<{ items: ComandaItem[] }>(await api(`/api/comandas/comandas/${comanda.id}`, {}, admin!));
  check('comanda original ficou com 2 itens após split', comandaAfterSplit.items.length === 2);
  const newComandaAfterSplit = await unwrap<{ items: ComandaItem[] }>(await api(`/api/comandas/comandas/${split.newComandaId}`, {}, admin!));
  ...
  const comandaAfterMerge = await unwrap<{ items: ComandaItem[] }>(await api(`/api/comandas/comandas/${comanda.id}`, {}, admin!));
  check('comanda voltou a ter 3 itens após merge', comandaAfterMerge.items.length === 3);
  const sourceAfterMerge = await unwrap<{ status: string }>(await api(`/api/comandas/comandas/${split.newComandaId}`, {}, admin!));
  ...
  const comandaFechada = await unwrap<{ status: string; sale_id: number }>(await api(`/api/comandas/comandas/${comanda.id}`, {}, admin!));
  check('comanda marcada como fechada com sale_id', comandaFechada.status === 'fechada' && comandaFechada.sale_id === closed.saleId);
  const statusAfterClose = await unwrap<{ id: number; status: string }[]>(await api('/api/comandas/tables/status', {}, admin!));
  ...
  const comandaC = await unwrap<{ id: number }>(openC);
  ...
  const comandaCAfter = await unwrap<{ status: string; sale_id: number | null }>(await api(`/api/comandas/comandas/${comandaC.id}`, {}, admin!));
  ...
  const statusAfterCancel = await unwrap<{ id: number; status: string }[]>(await api('/api/comandas/tables/status', {}, admin!));
