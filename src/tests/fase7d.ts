...
async function phase2(): Promise<void> {
  ...
  const a: Machine = { name: 'A', base: `http://localhost:${portA}`, proc: spawnProc('machineA', 'src/dev.ts', {
    KATSU_DB_PATH: path.join(SCRATCH, 'machineA.db'), KATSU_PORT: String(portA), KATSU_SYNC_SERVER_URL: cloudUrl, KATSU_MACHINE_ID: 'test-machine-a-7d',
  }) };
  const b: Machine = { name: 'B', base: `http://localhost:${portB}`, proc: spawnProc('machineB', 'src/dev.ts', {
    KATSU_DB_PATH: path.join(SCRATCH, 'machineB.db'), KATSU_PORT: String(portB), KATSU_SYNC_SERVER_URL: cloudUrl, KATSU_MACHINE_ID: 'test-machine-b-7d',
  }) };
