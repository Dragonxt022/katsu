...
## Fase 6 (planejada, sem código ainda) — Cardápio online por empresa, hospedado na cloud
...
Com Food Service, Comandas & Mesas e o acesso pela rede local implementados e verificados de ponta a ponta (testes automatizados + `tsc` limpos + passe completo num navegador real via Playwright, clicando pelo fluxo Mesa → Comanda → Cozinha → PDV → venda, e pelos cargos restritos Garçom/Cozinha), **todo o roadmap do documento de arquitetura original está coberto**, mais a extensão de rede local pedida nesta sessão. O cardápio online (Fase 6 acima) fica planejado, não implementado. Qualquer trabalho depois disso é escopo novo, a ser levantado com o usuário quando surgir.
