...
6. Manual (sem navegador disponível no ambiente desta sessão — feito via `npm run dev` + `curl` autenticado com cookie de sessão real, cobrindo as páginas HTML que os testes automatizados acima não tocam, já que aqueles só batem na API): login, liga `comandas.mesas`+`foodservice.cozinha`, e bateu em `/app/comandas/mesas`, `/app/comandas/mesas/:id`, `/app/foodservice/cozinha`, `/app/foodservice/roteamento` e `/app/store/pdv?comandaId=`. **Achou um 4º bug real**, só visível ao renderizar as páginas (nenhum teste de API o pegava): as 4 views novas (`comandas-mesas.ejs`, `comandas-detalhe.ejs`, `foodservice-cozinha.ejs`, `foodservice-routing.ejs`) incluíam os partials compartilhados com caminho relativo `include('../../../store/views/partials/theme-init')`/`.../nav` — mas o Express está configurado com um array de `views` (`core views` + `views` de cada módulo, ver `core/server.ts`), então o padrão correto em todo o resto do código é sempre `include('partials/theme-init')`/`include('partials/nav', {...})` sem prefixo relativo (confirmado em `commercial`, `dre`, etc.); as 4 páginas novas quebravam com 500 `Could not find the include file`. Corrigido nas 4 views. Depois do fix, as 5 páginas renderizaram 200 sem erro no log do servidor, incluindo o carrinho pré-carregado com o banner "Fechando Comanda" em `/app/store/pdv?comandaId=`. Não cobre clique-a-clique real no navegador (drag/click do Alpine.js) — só renderização server-side e chamadas de API; um passe visual num navegador real é o próximo passo recomendado antes de uso em produção.

## Depois desta fase

Com Food Service e Comandas & Mesas implementados e verificados (testes automatizados + `tsc` limpos; falta só o passe manual no navegador do item 6 acima), **todo o roadmap do documento de arquitetura original está coberto** (Núcleo, Módulos, Capabilities, Tipos de Produto, Catálogo Avançado completo, Food Service, Comandas & Mesas). Qualquer trabalho depois disso é escopo novo, a ser levantado com o usuário quando surgir.
